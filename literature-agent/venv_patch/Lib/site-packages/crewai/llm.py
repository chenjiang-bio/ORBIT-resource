        ...
        try:
            ...
            for chunk in litellm.completion(**params):
                ...
                if chunk_content is not None:
                    ...
                    # --- Check for stop words using sliding window approach
                    if self.stop:
                        # Calculate the maximum stop word length for efficient checking
                        max_stop_len = max(len(s) for s in self.stop)
                        
                        # Only check the window: (previous content - max_stop_len) to end
                        # This ensures we catch stop words that might span across chunks
                        check_start = max(0, len(full_response) - len(chunk_content) - max_stop_len)
                        check_window = full_response[check_start:]
                        ...
                        for stop_word in self.stop:
                            # Search for stop word in the check window
                            pos = check_window.find(stop_word)
                            if pos != -1:
                                # Convert window position to full_response position
                                absolute_pos = check_start + pos
                                
                                # Track the earliest occurrence
                                if earliest_stop_pos is None or absolute_pos < earliest_stop_pos:
                                    earliest_stop_pos = absolute_pos
                                    earliest_stop_word = stop_word
                        
                        # If a stop word was found, truncate and stop streaming
                        if earliest_stop_pos is not None:
                            full_response = full_response[:earliest_stop_pos]
                            logging.info(
                                f"Stop word '{earliest_stop_word}' detected at position {earliest_stop_pos}. "
                                f"Stopping stream."
                            )
                            # Break out of the chunk processing loop
                            break
