...
class CrewAgentExecutor(CrewAgentExecutorMixin):
    ...
    def _invoke_loop(self) -> AgentFinish:
        ...
        wait_sec = int(os.getenv("AGENT_REQUEST_WAIT_SEC", "45"))
        last_request_time = time.time() - wait_sec
        while not isinstance(formatted_answer, AgentFinish):
            try:
                ...
                if  time.time() - last_request_time < wait_sec:
                    sleep_time = wait_sec - (time.time() - last_request_time)
                    print(f"Sleep {sleep_time} for rate limit.")
                ...
                min_index = len(answer)
                for stop_item in self.llm.stop:
                    index = answer.find(stop_item)
                    if index >= 0 and index < min_index:
                        min_index = index
                answer = answer[:min_index]
                formatted_answer = process_llm_response(answer, self.use_stop_words)

                # Support single AgentAction or multi-action list from parse().
                actions: list[AgentAction] = []
                if isinstance(formatted_answer, list):
                    actions = [
                        a for a in formatted_answer if isinstance(a, AgentAction)
                    ]
                elif isinstance(formatted_answer, AgentAction):
                    actions = [formatted_answer]
                ...
                if actions:
                    ...
                    for i, action in enumerate(actions):
                        # Carry forward prior observations so multi-tool turns
                        # keep full tool history in the message text.
                        if i > 0 and isinstance(formatted_answer, AgentAction):
                            action.text = formatted_answer.text
