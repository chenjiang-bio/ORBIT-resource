...
def parse(text: str) -> AgentAction | list[AgentAction] | AgentFinish:
    """Parse agent output text into AgentAction(s) or AgentFinish.

    Expects output to be in one of two formats.

    If the output signals that an action should be taken,
    should be in the below format. This will result in an AgentAction
    (single match) or list[AgentAction] (multiple matches) being returned.
    ...
    Returns:
        AgentAction, list[AgentAction], or AgentFinish based on the content.
    ...
    action_matchs = ACTION_INPUT_REGEX.findall(text)
    ...
    if action_matchs and len(action_matchs) > 0:
        actions = []
        for action_match in action_matchs:
            action = action_match[0].strip()
            clean_action = _clean_action(action)

            action_input = action_match[1].strip()
        ...
        # Single action: return scalar for stock executor compatibility.
        # Multiple actions: return list for multi-tool turns.
        if len(actions) == 1:
            return actions[0]
        return actions
