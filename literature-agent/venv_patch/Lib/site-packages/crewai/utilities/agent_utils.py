...
from typing import TYPE_CHECKING, Any, Final, Literal, TypedDict, List
...
def handle_max_iterations_exceeded(
    ...
    if isinstance(formatted_answer, list) and formatted_answer:
        last = formatted_answer[-1]
        last_text = getattr(last, "text", "") or ""
        assistant_message = last_text + f"\n{i18n.errors('force_final_answer')}"
    elif formatted_answer and hasattr(formatted_answer, "text"):
        assistant_message = (
...
def format_answer(answer: str) -> List[AgentAction] | AgentFinish:
