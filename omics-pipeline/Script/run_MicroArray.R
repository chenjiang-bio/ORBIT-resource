...
#                                         # (DEG/GSVA outputs rename those cols to Source)
#
# Usage:
...
run_one_comparison <- function(i, comparisons, samples_info, exprs_mat, gse, gse_dir) {
  ...
  # DEG/GSVA per-sample columns after regulation/Regulation use Source labels
  geo.data.current <- rename_sample_cols_to_source(geo.data.current, meta_cur)
  # Keep meta_cur$Source in sync with (possibly make.names-unique) colnames
  meta_cur$Source <- colnames(geo.data.current)
  ...
  if (nrow(info_Control) >= 2) {
    AveExpr_Control <- rowMeans(geo.data.current.ave[, info_Control$Source, drop = FALSE])
  } else {
    AveExpr_Control <- geo.data.current.ave[, info_Control$Source]
  }
  if (nrow(info_Treatment) >= 2) {
    AveExpr_Case <- rowMeans(geo.data.current.ave[, info_Treatment$Source, drop = FALSE])
  } else {
    AveExpr_Case <- geo.data.current.ave[, info_Treatment$Source]
  }
