...
# _SUCCESS marker when all steps finish.
...
#' Rename matrix/data.frame columns from Sample IDs to Source labels.
#'
#' Source names after the regulation/Regulation column.
#' Columns not present in samples_info$Sample are left unchanged.
#' @param mat matrix or data.frame
#' @param samples_info data.frame with Sample and Source
#' @param unique_names if TRUE, run make.names(..., unique=TRUE) on remapped cols
#' @return mat with renamed colnames
rename_sample_cols_to_source <- function(mat, samples_info, unique_names = TRUE) {
  if (is.null(mat) || is.null(colnames(mat)) || length(colnames(mat)) == 0) {
    return(mat)
  }
  if (is.null(samples_info) || !"Sample" %in% names(samples_info) ||
      !"Source" %in% names(samples_info)) {
    return(mat)
  }
  sample_to_source <- stats::setNames(
    as.character(samples_info$Source),
    as.character(samples_info$Sample)
  )
  cn <- as.character(colnames(mat))
  hit <- cn %in% names(sample_to_source)
  if (!any(hit)) {
    return(mat)
  }
  new_cn <- cn
  new_cn[hit] <- unname(sample_to_source[cn[hit]])
  if (isTRUE(unique_names) && anyDuplicated(new_cn)) {
    new_cn <- make.names(new_cn, unique = TRUE)
  }
  colnames(mat) <- new_cn
  mat
}
