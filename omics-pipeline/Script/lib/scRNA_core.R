...
analyze_scRNA_cohort <- function(scRNA_all, samples_info, gse.number, organism,
  ...
  # alt: classic Seurat + IntegrateLayers (disabled)
  if (FALSE) {
    # layer
  ...
  # ---- 3. Neighbors, clusters, UMAP / t-SNE ----
  ...
  for (res in cluster_resolutions) {
    ...
    if (FALSE) {
      # CellMarker-based rename (disabled)
    ...
    # 5.2 Cell clusters / cell types table
    if (TRUE) {
    ...
    ### 5.5 Group DEGs (multi mode)
    if (isTRUE(do_multi_de)) {
      # FindMarkers path (<2 replicates per group)
    if(nrow(info_Control) < 2 | nrow(info_Treatment) < 2) {
      ...
      # muscat path (>=2 replicates per group)
            ...
            if (!is.null(sim_result)) {
              ...
              if (!is.null(slingPseudotime(sim_result))) {
                ...
                if (length(pseudotime_vals) == ncol(sim_result)) {
                  ...
                  pseudotime_df[[start]] <- pseudotime_vals
                } else {
    ...
    ### 5.7 Pseudobulk DE + enrichment (multi mode)
    if (isTRUE(do_multi_de)) {
    if (TRUE) {
      ...
      # 5.7.1 Bulk-style DEG on pseudobulk counts
      DEG_func <- function(current_counts){
        current_counts <- current_counts[, as.character(group_data$Sample), drop = FALSE]
        # Design matrix uses Source labels (same as bulk RNA-seq)
        src_names <- make.names(as.character(group_data$Source), unique = TRUE)
        colnames(current_counts) <- src_names
        gd <- as.data.frame(group_data, stringsAsFactors = FALSE)
        gd$Source <- src_names
        rownames(gd) <- src_names
        ctrl_cols <- src_names[as.character(gd$Group) == as.character(group1)]
        case_cols <- src_names[as.character(gd$Group) == as.character(group2)]
        ...
        if (deg_method == "DESeq2") {
          ...
          if (exists("run_edger_low_replicate", mode = "function")) {
            DEG <- run_edger_low_replicate(current_counts, gd$Group, bcv_value, group1, group2)
          } else {
            group <- factor(gd$Group, levels = c(group1, group2))
          ...
          conditions <- factor(gd$Group, levels = c(group1, group2))
...

        {
          ...
          # AveExpr_* = group means on CPM; sample cols (Source names) = gene-wise z-scores of CPM.
          y_expr <- edgeR::DGEList(counts = current_counts, group = gd$Group)
          ...
          if (length(ctrl_cols) >= 2) {
            AveExpr_Control <- rowMeans(DEG_expr[, ctrl_cols, drop = FALSE])
          } else {
            AveExpr_Control <- DEG_expr[, ctrl_cols]
          }
          if (length(case_cols) >= 2) {
            AveExpr_Case <- rowMeans(DEG_expr[, case_cols, drop = FALSE])
          } else {
            AveExpr_Case <- DEG_expr[, case_cols]
          }
      ...
      # 5.7.2 Enrichment helpers (GO / KEGG / CellMarker)
      {
        # save enrichment CSV
        ...
        # pathway network tables + optional cnet/emap plots
        ...
        # run enrichment stack (up / down / all diff)
        enrich_ana_func <- function(DEG_significant){
          ...
          # GO / KEGG enrichment (all diff) + network plots
      ...
      # safe_analysis wrapper (catch errors per step)
      ...
      # GSVA orchestrator (GO/KEGG + optional ssGSEA)
      GSVA_func <- function(current_counts) {
        current_counts <- current_counts[, as.character(group_data$Sample), drop = FALSE]
        # Per-sample z-score columns after Regulation use Source (not Sample/GSM)
        colnames(current_counts) <- make.names(as.character(group_data$Source), unique = TRUE)
        ...
        # GO + KEGG GSVA, then limma contrast
      ...
      # 5.7.5 Pseudobulk per cell type
      ...
      # pseudobulk by scType
      Idents(scRNA_subset_inter) <- "celltype_scType"
      pseudobulk_ana_func("celltype_scType")
      # pseudobulk by SingleR
      Idents(scRNA_subset_inter) <- "celltype_SingleR"
